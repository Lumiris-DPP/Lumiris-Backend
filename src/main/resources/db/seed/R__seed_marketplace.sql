...
-- ── 0. S'assurer que l'artisan démo principal existe ─────────────────────────
...
SELECT id, 'Atelier Démo', 'Atelier Démo', 'atelier-demo', 'Paris', 'Île-de-France', 'Pro', 100, NOW()
...
WHERE u.email = 'artisan@lumiris.com'
    -- uq_marketplace_products_dpp est un index UNIQUE *partiel* : le prédicat doit être
    -- répété ici, sinon Postgres ne peut pas l'inférer (42P10).
    ON CONFLICT (dpp_form_id) WHERE dpp_form_id IS NOT NULL DO NOTHING;
